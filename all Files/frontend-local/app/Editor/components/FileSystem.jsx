...
export default function FileSystem() {
  ...
  if(socketAddress != undefined){
    // uset this if hosting
    // socket = io(`http://34.67.228.203:${socketAddress}`)

    // use htis if on localhost
  ...
  const fetchData = async () => {
    if(socketAddress!=undefined){
      ...
      try{
        // if hosting use this and put the link address


        // let data = await fetch(`http://34.67.228.203:${socketAddress}/files`)


        // if localhost use this 
        let data = await fetch(`http://localhost:5000/files`)
